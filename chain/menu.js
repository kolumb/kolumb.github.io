...
var defaultNames = ['Первый','Второй','Третий','Четвёртый','Пятый','Шестой','Седьмой','Восьмой']
function getNames(n){
	players = []
...
document.querySelector('.options').addEventListener('click', function(e){
	...
	if(SpeechChk.checked) {
		...
		speech = SpeedChk.checked ? 'быстрая' : 'вкл'
...
function MakeURL(level, playersNames, mute, speech, voice) {
	level = level || 4
	playersNames = playersNames || ['Первый','Второй']
	speech = speech || ''
	var url = './'
	...
	if(mute) {
		...
		url+= speech ? '&речь=' + speech : ''
		url+= voice !== undefined ? '&голос=' + voice : ''
	}
	return url
}